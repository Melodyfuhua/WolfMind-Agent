...
from game_service import run_game_session, run_human_game_session
from core.human_agent import HumanInputChannel
...
class GameRuntime:
    ...
    mode: str = "ai"  # ai|human（纯 AI 对局 / 真人参与）
    ...
    human_channel: HumanInputChannel | None = None  # 真人对局的输入通道
...
def create_app() -> FastAPI:
    ...
    async def _run_game_async(
        game_id: str,
        stop_event: threading.Event | None,
        mode: str = "ai",
        channel: HumanInputChannel | None = None,
    ) -> None:
        with runtime.lock:
        ...
        try:
            if mode == "human" and channel is not None:
                log_path, experience_path = await run_human_game_session(
                    game_id=game_id, channel=channel, event_sink=bus.publish, stop_event=stop_event)
            else:
    ...
    def _thread_entry(
        ...
        try:
            task = loop.create_task(
                _run_game_async(game_id, stop_event, mode, channel))
            with runtime.lock:
    ...
    @app.post("/api/game/start_human", response_model=StartGameResponse)
    async def start_human_game() -> StartGameResponse:
        """开始一局有真人参与的游戏（8 AI + 1 人，随机座位与身份）。"""
        with runtime.lock:
            running = runtime.thread is not None and runtime.thread.is_alive()
        if running:
            raise HTTPException(
        ...
        game_id = _new_game_id()
        stop_event = threading.Event()
        # 人类输入通道：emit 直达事件总线（人类自己的请求/私密信息不经视角过滤）
        channel = HumanInputChannel(emit=bus.publish)
        t = threading.Thread(target=_thread_entry, args=(
            game_id, stop_event, "human", channel), daemon=True)
        with runtime.lock:
            runtime.stop_event = stop_event
            runtime.thread = t
            runtime.status = "running"
            runtime.game_id = game_id
            runtime.mode = "human"
            runtime.human_channel = channel
            runtime.last_error = None
            runtime.log_path = None
            runtime.experience_path = None
        t.start()

        bus.publish({"type": "system", "content": "已收到开始游戏请求（真人参与模式）"})
                        ...
                        if d is recv_task:
                            ...
                            if raw:
                                try:
                                    ...
                                    elif isinstance(msg, dict) and msg.get("type") == "human_input":
                                        # 真人提交了某一步的输入，转交给游戏线程唤醒等待
                                        req_id = msg.get("requestId")
                                        data = msg.get("data") or {}
                                        with runtime.lock:
                                            channel = runtime.human_channel
                                        if channel is not None and req_id:
                                            channel.submit(str(req_id), data)
                                except Exception:
                                    # 忽略非 JSON 文本
                                    pass
                        else:
